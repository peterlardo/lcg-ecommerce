...
export async function GET(_request: Request, context: { params: Promise<unknown> }) {
  ...
  const [point, orders, reservations, stocks, movements, openCash, sales, todaySales, weeklySales] = await Promise.all([
    prisma.pointOfSale.findUnique({ where: { id }, include: { managerUser: { select: { id: true, name: true, email: true } }, cashSessions: { orderBy: { openedAt: "desc" }, take: 50 } } }),
    ...
    prisma.order.aggregate({ where: { pointOfSaleId: id, status: { not: "CANCELLED" }, createdAt: { gte: startOfDay } }, _sum: { total: true }, _count: { id: true } }),
