...
interface Detail {
  point: PointOfSale
  orders: { id: string; orderNumber: string; customerName: string | null; total: number; status: string; createdAt: string }[]
  reservations: { id: string; client: string; type: string; date: string; heure: string; status: string; pointOfSaleId?: string }[]
  stocks: StockLine[]
  ...
  cashSessions: { id: string; openedAt: string; closingBalance: number | null; openingBalance: number; status: string }[]
  weeklyCash: { label: string; sessions: number; totalOpened: number; totalClosed: number }[]
}
      ...
      {showForm && (
        <form onSubmit={savePoint} className="rounded-xl border border-border bg-card p-5 shadow-card-soft">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {([["name", "Nom", true], ["code", "Code automatique", true], ["address", "Adresse", true], ["city", "Ville", true], ["phone", "Téléphone", false], ["managerName", "Nom du responsable", false]] as const).map(([key, label, required]) => (
              <label key={key} className="text-xs font-medium text-muted-foreground">{label}<input required={required} value={form[key]} readOnly={key === "code" && !editing} onChange={(event) => setForm({ ...form, [key]: event.target.value })} className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring/30" /></label>
            ))}
            <label className="text-xs font-medium text-muted-foreground">Utilisateur responsable
              <select value={form.managerUserId} onChange={(event) => setForm({ ...form, managerUserId: event.target.value })} className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground">
          ...
          <div className="mt-5 flex justify-end gap-3">
            <button type="button" onClick={() => setShowForm(false)} className="rounded-lg border border-border px-4 py-2 text-sm text-muted-foreground">Annuler</button>
            <button disabled={saving} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50">{saving ? "Enregistrement..." : "Enregistrer"}</button>
          </div>
        </form>
      )}
        ...
        <section className="space-y-3">
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Rechercher..." className="w-full rounded-lg border border-input bg-background py-2.5 pl-9 pr-3 text-sm text-foreground" />
            </div>
            <select value={status} onChange={(event) => setStatus(event.target.value)} className="rounded-lg border border-input bg-background px-2 text-sm text-foreground">
          ...
          {filteredPoints.map((point) => (
            <button key={point.id} onClick={() => { setSelectedId(point.id); void loadDetail(point.id) }} className={`w-full rounded-xl border bg-card p-4 text-left transition ${selectedId === point.id ? "border-primary ring-2 ring-primary/20" : "border-border hover:border-primary/40"}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="rounded-lg bg-primary/10 p-2.5 text-primary"><Store className="h-5 w-5" /></div>
                  <div><p className="font-semibold text-foreground">{point.name}</p><p className="mt-1 text-xs text-muted-foreground">{point.code} · {point.city}</p></div>
                </div>
                <span className={`h-2.5 w-2.5 rounded-full ${point.isActive ? "bg-green-500" : "bg-gray-400"}`} />
              </div>
              <div className="mt-4 flex justify-between text-xs text-muted-foreground">
                <span>{formatPrice(stats[point.id] || 0)}</span>
                <span>{point._count.orders} vente(s)</span>
              </div>
            </button>
          ))}
          {!loading && filteredPoints.length === 0 && <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">Aucun point trouvé.</div>}
        </section>
          ...
          <section className="min-w-0 rounded-xl border border-border bg-card shadow-card-soft">
            <div className="border-b border-border p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => startEdit(selectedPoint)} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm text-muted-foreground hover:bg-muted"><Edit3 className="h-4 w-4" /> Modifier</button>
                  <button onClick={() => togglePoint(selectedPoint)} className="rounded-lg border border-border px-3 py-2 text-sm text-muted-foreground hover:bg-muted">{selectedPoint.isActive ? "Désactiver" : "Activer"}</button>
                </div>
              </div>
              <div className="mt-5 flex gap-1 overflow-x-auto border-b border-border">
                {([["overview", "Vue générale"], ["stock", "Stock & transferts"], ["cash", "Caisse"], ["history", "Historique"]] as const).map(([value, label]) => (
                  <button key={value} onClick={() => setTab(value)} className={`border-b-2 px-3 py-2 text-sm font-medium ${tab === value ? "border-primary text-primary" : "border-transparent text-muted-foreground"}`}>{label}</button>
                ))}
              </div>
            </div>
            ...
            {tab === "overview" && (
              <div className="space-y-5 p-5">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  ...
                  <div className="rounded-lg bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Ventes totales</p><p className="mt-2 text-xl font-bold text-foreground">{detail.summary.orders}</p></div>
                  <div className="rounded-lg bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Articles en stock</p><p className="mt-2 text-xl font-bold text-foreground">{detail.stocks.reduce((sum, item) => sum + item.quantity, 0)}</p></div>
                </div>
                {detail.weeklyHistory && detail.weeklyHistory.length > 0 && (
                  <div className="rounded-lg border border-border bg-card p-4">
                    <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Historique hebdomadaire</p>
                    <div className="space-y-2">
                      {detail.weeklyHistory.filter((w) => w.revenue > 0 || w.orders > 0).slice(0, 8).map((week, i) => {
                ...
                <div>
                  <div className="mb-3 flex items-center justify-between">
                    <h3 className="font-semibold text-foreground">Dernières ventes</h3>
                    <Link href={`/admin/ventes?pointOfSaleId=${selectedId}`} className="text-xs font-semibold text-primary">Ouvrir la caisse <ChevronRight className="inline h-3 w-3" /></Link>
                  </div>
                  <div className="divide-y divide-border rounded-lg border border-border">
                    {detail.orders.slice(0, 6).map((order) => (
                      <div key={order.id} className="flex items-center justify-between gap-3 p-3 text-sm">
                        <div><p className="font-medium text-foreground">{order.orderNumber}</p><p className="text-xs text-muted-foreground">{order.customerName || "Client"}</p></div>
                        <div className="text-right"><p className="font-semibold text-foreground">{formatPrice(order.total)}</p><p className="text-xs text-muted-foreground">{new Date(order.createdAt).toLocaleDateString("fr-FR")}</p></div>
                      </div>
                    ))}
                    {detail.orders.length === 0 && <p className="p-5 text-sm text-muted-foreground">Aucune vente rattachée.</p>}
                  </div>
                </div>
                <div>
                  <h3 className="mb-3 font-semibold text-foreground">Pré-commandes rattachées</h3>
                  <div className="divide-y divide-border rounded-lg border border-border">
                ...
                <form onSubmit={submitStock} className="rounded-lg border border-border p-4">
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                    <select required value={stockForm.variantId} onChange={(event) => setStockForm({ ...stockForm, variantId: event.target.value })} className="rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground">
                      <option value="">Produit / format</option>
                      {variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.product.name} · {variant.format}</option>)}
                    </select>
            ...
            {tab === "cash" && (
              <div className="space-y-5 p-5">
                ...
                <Link href={`/admin/caisse?pointOfSaleId=${selectedId}`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary">Voir le rapprochement caisse <ChevronRight className="h-4 w-4" /></Link>
                {detail.weeklyCash && detail.weeklyCash.filter((w) => w.sessions > 0).length > 0 && (
                  <div className="rounded-lg border border-border bg-card p-4">
                    <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Historique hebdomadaire des caisses</p>
                ...
                {detail.cashSessions && detail.cashSessions.length > 0 && (
                  <div className="rounded-lg border border-border bg-card p-4">
                    <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Dernières sessions</p>
                    <div className="divide-y divide-border">
                      {detail.cashSessions.slice(0, 10).map((session) => (
                        <div key={session.id} className="flex items-center justify-between gap-3 p-3 text-sm">
                          <div className="flex items-center gap-3">
                            {session.status === "OPEN" ? <Clock3 className="h-4 w-4 text-green-500" /> : <CheckCircle2 className="h-4 w-4 text-gray-400" />}
                            <div>
                              <p className="font-medium text-foreground">{new Date(session.openedAt).toLocaleString("fr-FR")}</p>
                              <p className="text-xs text-muted-foreground">Fond: {formatPrice(session.openingBalance)}</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className={`rounded-full px-2 py-0.5 text-xs font-medium inline-block ${session.status === "OPEN" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}>{session.status === "OPEN" ? "Ouverte" : "Fermée"}</p>
                            {session.closingBalance !== null && <p className="mt-1 text-xs text-muted-foreground">Clôture: {formatPrice(session.closingBalance)}</p>}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="space-y-4 p-5">
                ...
                <div className="divide-y divide-border rounded-lg border border-border">
                  {detail.movements.map((movement) => (
                    <div key={movement.id} className="flex items-center justify-between gap-3 p-3 text-sm">
                      <div><p className="font-medium text-foreground">{movement.variant.product.name} · {movement.variant.format}</p><p className="text-xs text-muted-foreground">{movement.reason || movement.type}</p></div>
                      <div className="text-right">
                        <p className={`font-semibold ${movement.type.includes("OUT") || movement.type === "SALE" ? "text-red-600" : "text-green-600"}`}>{movement.type.includes("OUT") || movement.type === "SALE" ? "-" : "+"}{movement.quantity}</p>
                        <p className="text-xs text-muted-foreground">{new Date(movement.createdAt).toLocaleString("fr-FR")}</p>
                      </div>
                    </div>
                  ))}
                  {detail.movements.length === 0 && <p className="p-5 text-sm text-muted-foreground">Aucun mouvement enregistré.</p>}
                </div>
              </div>
            )}
          </section>
