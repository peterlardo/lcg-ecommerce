    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Profils de rôles</h1>
            <p className="text-sm text-muted-foreground">Créez et gérez les profils de rôles, puis assignez-les aux utilisateurs.</p>
          </div>
        </div>
        <button onClick={() => setShowCreate(!showCreate)} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:opacity-90">
          <Plus className="h-4 w-4" /> Nouveau rôle
        </button>
      </div>

      {showCreate && (
        <form onSubmit={createRole} className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <input required placeholder="Ex: RESPONSABLE_VENTE" value={newRole.key} onChange={(e) => setNewRole({ ...newRole, key: e.target.value.toUpperCase().replace(/[^A-Z0-9_]/g, "_") })} className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground uppercase" />
            </div>
            <div>
              <label className="mb-1 block text-xs text-muted-foreground">Libellé</label>
              <input required placeholder="Ex: Responsable de vente" value={newRole.label} onChange={(e) => setNewRole({ ...newRole, label: e.target.value })} className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground" />
            </div>
            <div>
              <label className="mb-1 block text-xs text-muted-foreground">Description</label>
              <input placeholder="Description du rôle" value={newRole.description} onChange={(e) => setNewRole({ ...newRole, description: e.target.value })} className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground" />
            </div>
            <div>
      ...
      <div>
        <h2 className="text-lg font-semibold text-foreground mb-3">Permissions par rôle</h2>
        <p className="text-sm text-muted-foreground mb-4">Configurez les permissions par défaut pour chaque rôle. Ces permissions s&apos;appliquent aux modules accessibles.</p>
      </div>
          ...
          {Object.values(roleData).map((rd) => {
            ...
            return (
              <div key={rd.role} className="rounded-xl border border-border bg-card shadow-card-soft overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => setExpandedRole(isExpanded ? null : rd.role)}>
                  <div className="flex items-center gap-3">
                    {profile && <span className="h-3 w-3 rounded-full" style={{ backgroundColor: profile.color || "#6b7280" }} />}
                    <span className="text-sm font-semibold text-foreground">{profile?.label || rd.role}</span>
                    <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <Users className="h-3.5 w-3.5" /> {profile?.userCount ?? 0} utilisateur(s)
                    </span>
                    <span className="text-xs text-muted-foreground">{activeCount} module(s)</span>
                  </div>
                  <div className="flex items-center gap-2">
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-muted/50 text-xs text-muted-foreground">
                          <tr>
                            ...
                            <th className="px-3 py-2.5 text-center font-medium"><button onClick={() => toggleAllModules(rd.role, "canView", !allView)} className="hover:text-foreground transition-colors">Voir {allView ? "✓" : ""}</button></th>
                            <th className="px-3 py-2.5 text-center font-medium"><button onClick={() => toggleAllModules(rd.role, "canCreate", !allCreate)} className="hover:text-foreground transition-colors">Créer {allCreate ? "✓" : ""}</button></th>
                            <th className="px-3 py-2.5 text-center font-medium"><button onClick={() => toggleAllModules(rd.role, "canEdit", !allEdit)} className="hover:text-foreground transition-colors">Modifier {allEdit ? "✓" : ""}</button></th>
                            <th className="px-3 py-2.5 text-center font-medium"><button onClick={() => toggleAllModules(rd.role, "canDelete", !allDelete)} className="hover:text-foreground transition-colors">Supprimer {allDelete ? "✓" : ""}</button></th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-border/50">
                          {rd.permissions.map((perm) => (
                            <tr key={perm.module} className="hover:bg-muted/20">
                              <td className="px-5 py-2.5 font-medium text-foreground">{moduleLabels[perm.module] || perm.module}</td>
