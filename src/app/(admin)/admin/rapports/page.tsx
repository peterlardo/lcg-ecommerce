    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {!loading && data && (
          <div className="flex items-center gap-2">
            <button onClick={exportPDF} className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-100 transition-colors">
              <Download className="h-4 w-4" /> PDF
            </button>
            <button onClick={exportExcel} className="inline-flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm font-medium text-green-700 hover:bg-green-100 transition-colors">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex gap-1 rounded-lg border border-gray-200 bg-gray-100 p-1">
          {periods.map((p) => (
            <button key={p.id} onClick={() => handlePeriodChange(p.id)} className={`rounded-md px-4 py-1.5 text-sm font-medium transition-colors ${period === p.id ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
              {p.label}
            </button>
          ))}
        </div>
      ...
      <div className="flex gap-1 overflow-x-auto rounded-lg border border-gray-200 bg-gray-100 p-1">
        {tabList.map((t) => {
          const Icon = t.icon
          return (
            <button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-2 whitespace-nowrap rounded-md px-4 py-2 text-sm font-medium transition-colors ${tab === t.id ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
        ...
        <>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <section className="rounded-xl border border-gray-200 bg-white p-5">
                  <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-gray-900"><BarChart3 className="h-4 w-4" /> CA (7j)</h2>
                ...
                <section className="rounded-xl border border-gray-200 bg-white p-5">
                  <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-gray-900"><TrendingUp className="h-4 w-4" /> Ventes ({data?.periodLabel ?? "30j"})</h2>
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                <section className="rounded-xl border border-gray-200 bg-white p-5">
                  <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-gray-900"><Package className="h-4 w-4" /> Top produits</h2>
                  <div className="space-y-2">{(data?.topProducts ?? []).slice(0, 8).map((p: any) => <div key={p.name} className="flex items-center justify-between rounded-lg bg-gray-50 p-3 text-sm"><span className="font-medium text-gray-800">{p.name}</span><div className="text-right"><span className="font-semibold">{p.quantity}</span><p className="text-xs text-gray-500">{formatPrice(p.revenue)}</p></div></div>)}</div>
                </section>
                <section className="rounded-xl border border-gray-200 bg-white p-5">
                  <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-gray-900"><WalletCards className="h-4 w-4" /> Paiements ({data?.periodLabel ?? "30j"})</h2>
                  <div className="space-y-2">{(data?.paymentBreakdown30 ?? []).map((p: any) => <div key={p.method} className="flex items-center justify-between rounded-lg bg-gray-50 p-3 text-sm"><div><p className="font-medium">{PAYMENT[p.method] ?? p.method}</p><p className="text-xs text-gray-500">{p.count} tx(s)</p></div><span className="font-semibold">{formatPrice(p.total)}</span></div>)}</div>
                </section>
                <section className="rounded-xl border border-gray-200 bg-white p-5">
                  <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-gray-900"><WalletCards className="h-4 w-4" /> Repartition</h2>
          ...
          {tab === "stocks" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <section className="rounded-xl border border-gray-200 bg-white p-5">
                  <h2 className="mb-4 text-sm font-semibold text-gray-900">Stock par categorie</h2>
                ...
                <section className="rounded-xl border border-gray-200 bg-white p-5">
                  <h2 className="mb-4 text-sm font-semibold text-gray-900">Alertes stock</h2>
                  <div className="space-y-2 max-h-64 overflow-y-auto">{(data?.stockAlerts ?? []).map((a: any) => <div key={a.variantId} className={`flex items-center justify-between rounded-lg p-3 text-sm ${a.stock <= 0 ? "bg-red-50" : "bg-yellow-50"}`}><div><p className="font-medium">{a.productName}</p><p className="text-xs text-gray-500">{a.format}</p></div><span className={`font-bold ${a.stock <= 0 ? "text-red-600" : "text-yellow-700"}`}>{a.stock}</span></div>)}</div>
                </section>
              </div>
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Toutes les variantes</h2>
                <div className="overflow-x-auto max-h-80"><table className="w-full text-sm"><thead className="sticky top-0 bg-gray-50/80"><tr><th className="px-4 py-2 text-left text-xs font-semibold uppercase text-gray-500">Produit</th><th className="px-4 py-2 text-left text-xs font-semibold uppercase text-gray-500">Format</th><th className="px-4 py-2 text-left text-xs font-semibold uppercase text-gray-500">Categorie</th><th className="px-4 py-2 text-right text-xs font-semibold uppercase text-gray-500">Stock</th></tr></thead><tbody className="divide-y divide-gray-100">{(data?.allStockVariants ?? []).map((v: any, i: number) => <tr key={i} className={v.stock <= 0 ? "bg-red-50/40" : v.stock <= 10 ? "bg-yellow-50/40" : ""}><td className="px-4 py-2">{v.productName}</td><td className="px-4 py-2 text-gray-600">{v.format}</td><td className="px-4 py-2 text-gray-600">{v.categoryName}</td><td className={`px-4 py-2 text-right font-bold ${v.stock <= 0 ? "text-red-600" : v.stock <= 10 ? "text-yellow-700" : ""}`}>{v.stock}</td></tr>)}</tbody></table></div>
              </section>
            </div>
          )}
          ...
          {tab === "appro" && (
            <div className="space-y-6">
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Approvisionnements ({data?.periodLabel ?? "Mois"})</h2>
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Mouvements</h2>
                <div className="space-y-2 max-h-80 overflow-y-auto">{(data?.supplyMovements ?? []).map((m: any) => <div key={m.id} className="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-2.5 text-sm"><div><p className="font-medium">{m.productName} ({m.format})</p><p className="text-xs text-gray-500">{m.reason || m.reference || "-"}</p></div><div className="flex items-center gap-3"><span className="rounded-full bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-700">{SUPPLY_LBL[m.type] ?? m.type}</span><span className="font-bold">+{m.quantity}</span><span className="text-xs text-gray-500"><Clock className="inline h-3 w-3" /> {new Date(m.createdAt).toLocaleDateString("fr-FR")}</span></div></div>)}</div>
              </section>
            </div>
          )}
          ...
          {tab === "commandes" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <section className="rounded-xl border border-gray-200 bg-white p-5">
                  <h2 className="mb-4 text-sm font-semibold text-gray-900">Commandes / jour</h2>
                ...
                <section className="rounded-xl border border-gray-200 bg-white p-5">
                  <h2 className="mb-4 text-sm font-semibold text-gray-900">Par statut</h2>
                  <div className="space-y-3">{(data?.ordersByStatus ?? []).map((item: any) => <div key={item.status} className="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-3 text-sm"><span className={`inline-block rounded-full px-2 py-0.5 text-xs font-semibold ${STATUS_BG[item.status] ?? "bg-gray-100 text-gray-700"}`}>{STATUS_LBL[item.status] ?? item.status}</span><div className="text-right"><p className="font-semibold">{item.count} cmd(s)</p><p className="text-xs text-gray-500">{formatPrice(item.total)}</p></div></div>)}</div>
                </section>
              </div>
            </div>
          )}
          ...
          {tab === "reservations" && (
            <div className="space-y-6">
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Pré-commandes / jour</h2>
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Dernières pré-commandes</h2>
                <div className="space-y-2 max-h-80 overflow-y-auto">{(data?.reservations ?? []).map((r: any) => <div key={r.id} className="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-2.5 text-sm"><div><p className="font-medium">{r.client}</p><p className="text-xs text-gray-500">{r.type} - {r.date} a {r.heure}</p></div><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${STATUS_BG[r.status] ?? "bg-gray-100"}`}>{STATUS_LBL[r.status] ?? r.status}</span></div>)}</div>
              </section>
            </div>
          )}
          ...
          {tab === "production" && (
            <div className="space-y-6">
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Production / jour</h2>
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Dernieres productions</h2>
                <div className="space-y-2 max-h-80 overflow-y-auto">{(data?.productionMovements ?? []).map((m: any) => <div key={m.id} className="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-2.5 text-sm"><div><p className="font-medium">{m.productName} ({m.format})</p><p className="text-xs text-gray-500">{m.reason || "-"}</p></div><div className="flex items-center gap-3"><span className="font-bold">+{m.quantity}</span><span className="text-xs text-gray-500">{new Date(m.createdAt).toLocaleDateString("fr-FR")}</span></div></div>)}</div>
              </section>
            </div>
          )}
          ...
          {tab === "lots" && (
            <div className="space-y-6">
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Tous les lots</h2>
                <div className="overflow-x-auto max-h-96"><table className="w-full text-sm"><thead className="sticky top-0 bg-gray-50/80"><tr>
                  ...
                  <th className="px-4 py-2 text-right text-xs font-semibold uppercase text-gray-500">Produit (qte)</th>
                  ...
                  <th className="px-4 py-2 text-center text-xs font-semibold uppercase text-gray-500">Allocations</th>
                  ...
                  <td className="px-4 py-2 font-mono text-xs font-semibold text-primary-700">{l.lotNumber}</td>
                  <td className="px-4 py-2">{l.productName}</td>
                  <td className="px-4 py-2 text-right">{l.initialQuantity}</td>
                  <td className={`px-4 py-2 text-right font-bold ${l.remainingQuantity === 0 ? "text-gray-400" : ""}`}>{l.remainingQuantity}</td>
                  <td className="px-4 py-2 text-center"><span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${l.status === "ACTIVE" ? "bg-green-100 text-green-700" : l.status === "EXPIRED" ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-600"}`}>{LOT_STATUS[l.status] ?? l.status}</span></td>
                  <td className="px-4 py-2 text-center text-gray-500">{l.allocationCount}</td>
                  <td className="px-4 py-2 text-gray-500">{new Date(l.productionDate).toLocaleDateString("fr-FR")}</td>
                  <td className="px-4 py-2 text-gray-500">{l.expiryDate ? new Date(l.expiryDate).toLocaleDateString("fr-FR") : "-"}</td>
                </tr>)}</tbody></table></div>
              </section>
            </div>
          )}
          ...
          {tab === "caisse" && (
            <div className="space-y-6">
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Sessions / jour</h2>
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                <h2 className="mb-4 text-sm font-semibold text-gray-900">Recapitulatif</h2>
                <div className="overflow-x-auto max-h-80"><table className="w-full text-sm"><thead className="sticky top-0 bg-gray-50/80"><tr><th className="px-4 py-2 text-left text-xs font-semibold uppercase text-gray-500">Date</th><th className="px-4 py-2 text-right text-xs font-semibold uppercase text-gray-500">Sessions</th><th className="px-4 py-2 text-right text-xs font-semibold uppercase text-gray-500">Ouverture</th><th className="px-4 py-2 text-right text-xs font-semibold uppercase text-gray-500">Cloture</th><th className="px-4 py-2 text-right text-xs font-semibold uppercase text-gray-500">Ecart</th></tr></thead><tbody className="divide-y divide-gray-100">{(data?.cashSessionsByDay ?? []).map((d: any) => <tr key={d.date}><td className="px-4 py-2">{d.name}</td><td className="px-4 py-2 text-right">{d.sessions}</td><td className="px-4 py-2 text-right">{formatPrice(d.openingTotal)}</td><td className="px-4 py-2 text-right">{d.closingTotal !== null ? formatPrice(d.closingTotal) : "-"}</td><td className={`px-4 py-2 text-right font-bold ${d.gap === null || d.gap === 0 ? "" : d.gap > 0 ? "text-blue-600" : "text-red-600"}`}>{d.gap !== null ? (d.gap > 0 ? "+" : "") + formatPrice(d.gap) : "-"}</td></tr>)}</tbody></table></div>
              </section>
            </div>
          )}
        </>
