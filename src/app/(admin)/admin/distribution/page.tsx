  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 sm:text-2xl">Distribution</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        <div className="rounded-xl border border-gray-200 bg-white p-3 sm:p-4"><p className="text-xs font-medium text-gray-500">À distribuer</p><p className="mt-1 text-lg font-bold text-gray-900 sm:text-xl">{distributionOrders.length}</p></div>
        <div className="rounded-xl border border-blue-200 bg-blue-50/40 p-3 sm:p-4"><p className="text-xs font-medium text-blue-700">Prêtes</p><p className="mt-1 text-lg font-bold text-blue-800 sm:text-xl">{readyOrders.length}</p></div>
        <div className="rounded-xl border border-orange-200 bg-orange-50/40 p-3 sm:p-4"><p className="text-xs font-medium text-orange-700">En cours</p><p className="mt-1 text-lg font-bold text-orange-800 sm:text-xl">{inTransit.length}</p></div>
        <div className="rounded-xl border border-green-200 bg-green-50/40 p-3 sm:p-4"><p className="text-xs font-medium text-green-700">Livrées</p><p className="mt-1 text-lg font-bold text-green-800 sm:text-xl">{deliveredToday}</p></div>
      </div>

      <div className="grid grid-cols-1 gap-4 sm:gap-6 xl:grid-cols-2">
        <section className="rounded-xl border border-gray-200 bg-white p-3 sm:p-5">
          <h2 className="mb-4 flex items-center gap-2 text-xs font-semibold text-gray-900 sm:text-sm"><PackageCheck className="h-4 w-4" /> Bons à préparer</h2>
            ...
            {!loading && distributionOrders.map((order) => (
              <div key={order.id} className="rounded-lg border border-gray-100 bg-gray-50 p-3 sm:p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex shrink-0 flex-col gap-2 sm:items-end"><span className="text-xs font-semibold text-gray-900 sm:text-sm">{formatPrice(order.total)}</span><span className="text-xs text-gray-500">{formatDate(order.createdAt)}</span>{order.status === "PROCESSING" && <button onClick={() => updateOrder(order.id, "READY")} className="rounded-lg bg-purple-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-purple-700">Marquer prête</button>}{order.status === "READY" && <button onClick={() => updateOrder(order.id, "OUT_FOR_DELIVERY")} className="rounded-lg bg-orange-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-orange-700">Sortie distribution</button>}</div>
                </div>
              </div>
            ))}
            {!loading && distributionOrders.length === 0 && <p className="rounded-lg bg-gray-50 p-3 text-center text-xs text-gray-500 sm:p-4 sm:text-sm">Aucun bon à distribuer.</p>}
          </div>
        </section>

        <section className="rounded-xl border border-gray-200 bg-white p-3 sm:p-5">
          <h2 className="mb-4 flex items-center gap-2 text-xs font-semibold text-gray-900 sm:text-sm"><Truck className="h-4 w-4" /> Distribution en cours</h2>
          <div className="space-y-3">
            {inTransit.map((delivery) => (
              <div key={delivery.id} className="rounded-lg bg-gray-50 p-3 sm:p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right"><p className="text-xs font-semibold text-gray-900 sm:text-sm">{formatPrice(delivery.total)}</p><p className="mt-1 text-xs text-gray-500">{delivery.agent || "Non assignée"}</p></div>
                </div>
              </div>
            ))}
            {inTransit.length === 0 && <p className="rounded-lg bg-gray-50 p-3 text-center text-xs text-gray-500 sm:p-4 sm:text-sm">Aucune distribution en cours.</p>}
          </div>
        </section>
      </div>
    </div>
